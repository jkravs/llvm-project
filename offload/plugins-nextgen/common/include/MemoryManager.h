...
public:
  ...
  ~MemoryManagerTy() {
    for (auto Itr = PtrToNodeTable.begin(); Itr != PtrToNodeTable.end();
      ...
      deleteOnDevice(Itr->second.Ptr, TARGET_ALLOC_DEFAULT);
    }
  }

  /// Allocate memory of size \p Size from target device. \p HstPtr is used to
  /// assist the allocation.
  void *allocate(size_t Size, void *HstPtr, TargetAllocTy Kind=TARGET_ALLOC_DEFAULT) {
  ...
  int free(void *TgtPtr, TargetAllocTy Kind = TARGET_ALLOC_DEFAULT) {
